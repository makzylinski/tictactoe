...
var randNumb = Math.floor(Math.random()*9);
var freeSpace = 9;
var turnCounter = 0;
var lock = false;
...
function putX(nr)
{
    if(lock == false)
    {
        if(isOccupied[nr] == false)
        {
            document.getElementById("d"+nr).innerHTML = '<img src="img/x.png" value="x" />';
            tick.play();
            isOccupied[nr] = true;
            position[nr] = "x";
            turnCounter++
        ...
        if(freeSpace>=2)
        {
            if(isOccupied[randNumb] == false)
            {
                document.getElementById("d"+randNumb).innerHTML = '<img src="img/o.png" value="o" />';
                isOccupied[randNumb] = true;
                position[randNumb] = "o";
                freeSpace--;
                validation();
            }
            ...
            {
                while(randNumb == nr || isOccupied[randNumb] == true)
                {
...
function validation()
{
    ...
    document.getElementById("turnCounter").innerHTML = "Aktualna runda " + turnCounter;    
}
